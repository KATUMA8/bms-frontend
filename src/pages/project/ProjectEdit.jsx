...
export default function ProjectEdit() {
  ...
  // 既存データとセレクトボックス用データの取得
  useEffect(() => {
    axios
      .get(`http://localhost:8080/api/projects/edit/${id}`)
      .then((res) => {
        const data = res.data;
        // バックエンドのレスポンス構造に合わせて調整（projectオブジェクトがある場合と直下の場合に対応）
        if (data.project) {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post(`http://localhost:8080/api/projects/edit/${id}`, projectForm)
      .then(() => {
